...
get_zscore <- function (ret_xts, wdsz) 
{
	#rolling cumulative returns 
	roll_ret <- rollapply(ret_xts, width = wdsz, FUN = function(x) {cumprod(1 +x)[wdsz, ] } )
	#write.zoo(d, file = "demo1.csv", sep=",")

	#rolling avg
	mean_ret <- rowMeans(roll_ret)
	#rolling std
	std_ret <- rowsd(roll_ret)

	#rolling z-score
	z_ret_g <- (roll_ret - mean_ret) / std_ret
}

get_wtd_adj_ret <-function (ret_xts_i,ret_xts_g, bm_xts, wdsz) {
#rolling window size

#f <-periodReturn(e,period='yearly')
z_ret_g <- get_zscore(ret_xts_g, wdsz)
z_ret_i <- get_zscore(ret_xts_i, wdsz)
...
jpeg(file=paste(wdsz, "mth_Global.jpg", sep = "_"), width = 1280, height = 1024)
...
#X11(width=12,height=8);
jpeg(file=paste(wdsz, "mth_International.jpg", sep = "_"), width = 1280, height = 1024)
